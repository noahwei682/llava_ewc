...
```
# 设置模型检查点目录
export CKPT_DIR=seed0-EVA02-CLIP-L-14-336-R15M-10ep

# 设置预训练数据的 JSON 文件
export PRETRAIN_JSON=blip_laion_cc_sbu_558k.json

# 设置 EVA 模型检查点路径
export EVA_CKPT=/blob/hwq/data/tune_logs/T_vit_256x8x2_lr1e-5_Rcc12mR_Rcc3m-2024_08_24-10/checkpoints/epoch_10/mp_rank_00_model_states.pt

# 进入指定的工作目录
cd /home/aiscuser/mycode/llava_aoqi

# 运行检查点目录的检查脚本
bash ./scripts/v1_5/eval/check_ckpt_dit.sh

# 设置 PyTorch CUDA 分配配置并运行所有脚本
PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True bash /home/aiscuser/mycode/llava_aoqi/scripts/v1_5/run_all.sh

# 运行指定的 Python 脚本
python /home/aiscuser/mycode/llava_aoqi/run_one.py
